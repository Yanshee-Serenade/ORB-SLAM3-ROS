...
System::System(const string &strVocFile, const string &strSettingsFile,
               const eSensor sensor, const std::string &loadAtlasFromFile,
               const bool bUseViewer, const int initFr,
               const string &strSequence)
    : mSensor(sensor), mStrLoadAtlasFromFile(loadAtlasFromFile),
...
bool System::LoadAtlas(int type) {
  ...
  if (type == TEXT_FILE) // File text
  {
    cout << "Starting to read the save text file " << pathLoadFileName.c_str()
         << endl;
  ...
  {
    cout << "Starting to read the save binary file " << pathLoadFileName.c_str()
         << endl;
    std::ifstream ifs(pathLoadFileName, std::ios::binary);
    if (!ifs.good()) {
      cout << "Load file not found" << endl;
